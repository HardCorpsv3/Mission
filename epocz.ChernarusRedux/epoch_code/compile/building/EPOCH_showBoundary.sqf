...
_model = "Sign_sphere100cm_F"; // marker model to use on rings
// Possible ones to use ::	Sign_sphere10cm_EP1  Sign_sphere25cm_EP1  Sign_sphere100cm_EP1 

_jammer = (nearestObjects [player, ["PlotPole_EPOCH"],15]) select 0;
_center = getPosASL _jammer;
...
if (!isNil "PP_Marks") then {
	if (((PP_Marks select 0) distance _jammer) < 10) then {  _obj = true; };
	_tmp = (PP_Marks select 0) distance _jammer;
...
if ((isNil "PP_Marks") && (!_obj)) then {
	...
	_obj = "Sign_sphere10cm_F" createVehicleLocal [0,0,0];	//	PARENT marker on pole
